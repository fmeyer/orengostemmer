...
import com.fernandomeyer.stemmer.Stemmer;
import com.fernandomeyer.stemmer.data.Tuple;
import com.fernandomeyer.stemmer.data.SuffixTree;
import com.fernandomeyer.stemmer.utils.XMLUtils;
...
public class OrengoStemmer extends Stemmer {
    ...
    private String applyRules(String st, SuffixTree<Rule> rules) {
        ...
        List<Tuple<String, Rule>> res = rules.getLongestSuffixesAndValues(st);

        for (int i = res.size() - 1; i >= 0; i--) {
            Tuple<String, Rule> r = res.get(i);
            String suffix = r.a;
            Rule rule = r.b;
    ...
    private void readRulesFromXML() throws PTStemmerException {
        ...
        try {
            ...
            document = builder.parse(OrengoStemmer.class.getResourceAsStream("OrengoStemmerRules.xml"));

        } catch (Exception e) {
            throw new PTStemmerException(
                    "Problem while parsing Orengo's XML stemming rules file.",
                    e);
        }
        ...
        for (Element step : XMLUtils.getChilds(root)) {
            if (!step.hasAttribute("name"))
                throw new PTStemmerException(
                        "Problem while parsing Orengo's XML stemming rules file: Invalid step.");
            ...
            for (Element rule : XMLUtils.getChilds(step)) {
                if (!rule.hasAttribute("size")
                        || !rule.hasAttribute("replacement")
                        || !rule.hasAttribute("suffix"))
                    throw new PTStemmerException(
                            "Problem while parsing Orengo's XML stemming rules file: Invalid rule in "
                                    + stepName + ".");
                ...
                for (Element exception : XMLUtils.getChilds(rule)) {
                    ...
                    if (!exception.getTagName().equals("exception")
                            || !exception.hasChildNodes())
                        throw new PTStemmerException(
                                "Problem while parsing Orengo's XML stemming rules file: Invalid exception in step "
                                        + stepName + ", rule " + suffix + ".");
                    exceptions.add(exception.getChildNodes().item(0)
                            .getNodeValue());
                }
                Rule r;
                try {
                    r = new Rule(Integer.parseInt(rule.getAttribute("size")),
                            rule.getAttribute("replacement"),
                            exceptions.toArray(new String[exceptions.size()]));
                } catch (NumberFormatException e) {
                    throw new PTStemmerException(
                            "Problem while parsing Orengo's XML stemming rules file: Missing or invalid rules properties on step "
                                    + stepName + ".", e);
                }
        ...
        if (pluralreductionrules == null || femininereductionrules == null
                || adverbreductionrules == null
                || augmentativediminutivereductionrules == null
                || nounreductionrules == null || verbreductionrules == null
                || vowelremovalrules == null)
            throw new PTStemmerException(
                    "Problem while parsing Orengo's XML stemming rules file: Missing steps.");
    }
